...
def make_bash_modified_video(name_file, caption_file, ori_videos_dir, tts_dir, modified_vid_dir):
    ori_videos_dir = ori_videos_dir + '/v_'
    tts_dir = tts_dir + '/v_'
    modified_vid_dir = modified_vid_dir + '/v_'
    ...
    for vid in video_key:
        ...
        out_path = modified_vid_dir + vid + tts_type[index_type] + '.mp4'
...
def extract_audio(name_file, ori_videos_dir, modified_vid_dir):
    f = open(name_file, )
    names = json.load(f)
    video_key = names['video_ids']
    ...
    bash_file = open('extract_original.sh', 'w')
    bash_file.write('#!/bin/bash\n')
    ori_aud_dir = './Original_Audio/v_'
    for vid in video_key:
        s = 'ffmpeg -i ' + ori_videos_dir + vid + '.mp4 -map 0:a -c copy ' + ori_aud_dir + vid + '.aac\n'
        bash_file.write(s)
    bash_file.close()

    # Extract modified video's audio
    modified_aud_dir = './Modified_Audio/v_'
    bash_file = open('extract_modified.sh', 'w')
    bash_file.write('#!/bin/bash\n')
    for vid in video_key:
        s = 'ffmpeg -i ' + modified_vid_dir + vid + '_pred.mp4 -map 0:a -c copy ' + modified_aud_dir + vid + '.aac\n'
        bash_file.write(s)
    bash_file.close()